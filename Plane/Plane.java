        ...
        try
		{
			...
			while(socket.isConnected())
			{
				...
				if(valueFromServer == 11)
				{
			        ...
			        runTime.exec(new String[] {"bash", "-c", "echo 3=20% > /dev/servoblaster"});
				}

				if(valueFromServer == 12)
				{
					System.out.println("Elevator 30%");
			        runTime.exec(new String[] {"bash", "-c", "echo 3=30% > /dev/servoblaster"});
				}

				if(valueFromServer == 13)
				{
					System.out.println("Elevator 40%");
			        runTime.exec(new String[] {"bash", "-c", "echo 3=40% > /dev/servoblaster"});
				}
				...
				if(valueFromServer == 15)
				{
			        ...
			        runTime.exec(new String[] {"bash", "-c", "echo 3=60% > /dev/servoblaster"});
				}

				if(valueFromServer == 16)
				{
					System.out.println("Elevator 70%");
			        runTime.exec(new String[] {"bash", "-c", "echo 3=70% > /dev/servoblaster"});
				}

				if(valueFromServer == 17)
				{
					System.out.println("Elevator 80%");
			        runTime.exec(new String[] {"bash", "-c", "echo 3=80% > /dev/servoblaster"});
				}
